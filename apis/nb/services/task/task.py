...
class Task(object):
    ...
    def __wait_for_task_complete(self, task_id=None, timeout=None):
        ...
        while not task_completed:
            if (time.time() > (start_time + timeout)):
                assert False, ("Task {0} didn't complete within {1} seconds".format(task_response.__dict__, timeout))
            ...
            if self.__is_task_success(task_response) or self.__is_task_failed(task_response):
                ...
                time.sleep(self.TASK_COMPLETION_POLL_INTERVAL)
        return task_response
    ...
    def __wait_for_task_failure(self, task_id, timeout=None):
        ...
        Returns: Returns the failure/success response of the task.
        """
        ...
        while not task_completed:
            if (time.time() > (start_time + timeout)):
                msg = "Task {0} didn't complete within {1} seconds".format(task_response.__dict__,
                                                                           timeout)
                assert False, msg
    ...
    def wait_for_task_complete(self, task_id_result, timeout=None):
        ...
        for retry in range(retries):
            try:
                ...
                if error_code == NOT_FOUND:
                    ...
                    assert False, "\nHTTP error code: {0}\nerror result: {1}".format(error_code, json.loads(error_result))
    ...
    def is_task_tree_success(self, task_id_result=None):
        ...
        for task in task_list_response:
            assert self.__is_task_success(task_response=task), ("Task failed, task response {0}".format(task.__dict__))
        return True
